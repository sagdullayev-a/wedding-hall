...
function getPasswordStrength(password: string): { score: number; label: string; color: string } {
  ...
  if (score <= 1) return { score: 20, label: 'Juda zaif', color: 'bg-red-500' }
  if (score === 2) return { score: 40, label: 'Zaif', color: 'bg-orange-500' }
  if (score === 3) return { score: 60, label: "O'rtacha", color: 'bg-amber-500' }
  if (score === 4) return { score: 80, label: 'Kuchli', color: 'bg-emerald-500' }
  return { score: 100, label: 'Juda kuchli', color: 'bg-emerald-600' }
}

const STEPS = [
  { id: 1, label: "Shaxsiy ma'lumotlar", icon: User },
  { id: 2, label: 'Hisob ma\'lumotlari', icon: Lock },
  { id: 3, label: 'Rol tanlash', icon: Shield },
]
  ...
  const validateStep = (step: number): string | null => {
    switch (step) {
      case 1:
        ...
        if (!form.phone.trim()) return 'Telefon raqam kiritilishi shart'
        return null
      case 2:
        if (!form.username.trim()) return 'Username kiritilishi shart'
        if (form.username.length < 3) return 'Username kamida 3 ta belgidan iborat bo\'lishi kerak'
        if (!form.password) return 'Parol kiritilishi shart'
        if (form.password.length < 6) return 'Parol kamida 6 ta belgidan iborat bo\'lishi kerak'
        if (form.password !== form.confirmPassword) return 'Parollar mos kelmayapti'
        return null
      case 3:
        return null
      default:
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label htmlFor="firstName" className="text-rose-900 dark:text-foreground">First Name</Label>
                    ...
                    <div className="space-y-2">
                      <Label htmlFor="lastName" className="text-rose-900 dark:text-foreground">Last Name</Label>
                      <div className="relative">
                        <User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-rose-300 dark:text-rose-500" />
                        <Input
                          id="lastName"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="email" className="text-rose-900 dark:text-foreground">Email</Label>
                    <div className="relative">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="phone" className="text-rose-900 dark:text-foreground">Phone</Label>
                    <div className="relative">
              ...
              {currentStep === 2 && (
                ...
                >
                  <div className="space-y-2">
                    <Label htmlFor="username" className="text-rose-900 dark:text-foreground">Username</Label>
                    <div className="relative">
                    ...
                    {form.password.length > 0 && (
                      ...
                      >
                        ...
                        <div className="flex flex-wrap gap-x-3 gap-y-1 text-[10px] text-muted-foreground">
                          <span className={form.password.length >= 6 ? 'text-emerald-500' : ''}>✓ Kamida 6 belgi</span>
                          <span className={/[A-Z]/.test(form.password) ? 'text-emerald-500' : ''}>✓ Katta harf</span>
                          <span className={/[0-9]/.test(form.password) ? 'text-emerald-500' : ''}>✓ Raqam</span>
                          <span className={/[^A-Za-z0-9]/.test(form.password) ? 'text-emerald-500' : ''}>✓ Maxsus belgi</span>
                        </div>
                      </motion.div>
                    )}
                  </div>
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="confirmPassword" className="text-rose-900 dark:text-foreground">Confirm Password</Label>
                    <div className="relative">
                      <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-rose-300 dark:text-rose-500" />
                      <Input
                        id="confirmPassword"
              ...
              {currentStep === 3 && (
                ...
                >
                  <div className="space-y-3">
                    <Label className="text-rose-900 dark:text-foreground text-base font-semibold">I am a</Label>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Looking for a venue</p>
                          </div>
                        </div>
                        {form.role === 'customer' && (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">Listing my hall</p>
                          </div>
                        </div>
                        {form.role === 'owner' && (
                          <div className="absolute top-2 right-2">
