...
class ApiClient {
  ...
  async getHallAutocomplete(query: string) {
    return this.request(`/halls/autocomplete?q=${encodeURIComponent(query)}`)
  }
  ...
  async checkFavorite(hallId: string) {
    return this.request(`/favorites/${hallId}`)
  }

  async addFavorite(hallId: string) {
    return this.request(`/favorites/${hallId}`, { method: 'POST' })
  }

  async removeFavorite(hallId: string) {
    return this.request(`/favorites/${hallId}`, { method: 'DELETE' })
  }
  ...
  // Update Profile
  async updateProfile(data: { firstName?: string; lastName?: string; email?: string; phone?: string; username?: string; avatarUrl?: string | null }) {
