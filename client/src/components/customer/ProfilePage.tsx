...
export default function ProfilePage() {
  const { user, token, navigateTo, logout, updateUser } = useAppStore()
  ...
  const handleAvatarUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error('Faqat JPG, JPEG, PNG va WEBP formatidagi rasmlar qabul qilinadi')
      return
    }
  ...
  const handleAvatarRemove = () => {
    setAvatarUrl('')
    toast.success("Avatar olib tashlandi. Saqlash tugmasini bosing.")
  }
  ...
  const handleSaveProfile = async () => {
    ...
    if (isCustomer) {
      if (!firstName.trim() || !lastName.trim()) {
        toast.error("Ism va familiyani to'ldiring")
        return
      }
    } else {
    ...
    try {
      ...
      const updatePayload = isCustomer
        ? {
            firstName,
            lastName,
            ...
            <CardContent className="p-6 -mt-12 relative">
              <div className="flex flex-col sm:flex-row items-start sm:items-end gap-4">
                <div className="relative group">
                  <Avatar className="h-24 w-24 border-4 border-white dark:border-gray-800 shadow-xl overflow-hidden bg-muted">
                    {(isEditing ? avatarUrl : user.avatarUrl) ? (
                      <img src={(isEditing ? avatarUrl : user.avatarUrl) || undefined} alt="Avatar" className="w-full h-full object-cover" />
                    ) : (
                      <AvatarFallback className="bg-gradient-to-br from-indigo-900 to-amber-500 text-white text-2xl font-bold">
                        {userInitials}
                      </AvatarFallback>
                    )}
                  </Avatar>
                  {isEditing && (
                    <div className="absolute inset-0 bg-black/50 rounded-full flex flex-col items-center justify-center gap-1 opacity-100 group-hover:bg-black/60 transition-all duration-200">
                      <label className="cursor-pointer text-white text-[10px] font-bold flex flex-col items-center p-2 hover:underline">
                      ...
                      {avatarUrl && (
                        <button
                          ...
                          className="text-red-300 hover:text-red-400 text-[10px] font-semibold hover:underline"
                        >
                          O'chirish
                        </button>
                      )}
              ...
              <Card className="border-rose-100 dark:border-rose-900/30">
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    ...
                    {!isEditing && (
                      <Button variant="ghost" size="sm" className="text-rose-600 hover:text-rose-700 hover:bg-rose-50 dark:hover:bg-rose-900/20" onClick={() => setIsEditing(true)}>
                  ...
                  {isEditing ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Ism (First Name)</label>
                          <Input
                            value={firstName}
                        ...
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block">Familiya (Last Name)</label>
                          <Input
                            value={lastName}
                        ...
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block flex items-center gap-1">
                            Email {user.role === 'customer' && <Lock className="w-3 h-3 text-slate-400" />}
                          </label>
                          {user.role === 'customer' ? (
                            <div className="relative">
                              <Input
                                ...
                                readOnly
                                disabled
                                className="border-rose-100 bg-slate-100/50 text-slate-500/80 cursor-not-allowed select-none pr-10 focus-visible:ring-0 focus-visible:ring-offset-0 focus:border-rose-100"
                        ...
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block flex items-center gap-1">
                            Telefon (Phone) {user.role === 'customer' && <Lock className="w-3 h-3 text-slate-400" />}
                          </label>
                          {user.role === 'customer' ? (
                            <div className="relative">
                              <Input
                        ...
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block flex items-center gap-1">
                            Foydalanuvchi nomi (Username) {user.role === 'customer' && <Lock className="w-3 h-3 text-slate-400" />}
                          </label>
                          {user.role === 'customer' ? (
                            <div className="relative">
                              <Input
                        ...
                        {/* Role (Always read-only for everyone) */}
                        <div>
                          <label className="text-xs font-medium text-muted-foreground mb-1 block flex items-center gap-1">
                            Rol (Role) <Lock className="w-3 h-3 text-slate-400" />
                          </label>
                          <div className="relative">
                            <Input
                      ...
                      <div className="flex justify-end gap-2 pt-2 border-t">
                        ...
                        >
                          ...
                          {updatingProfile ? 'Saqlanmoqda...' : 'Saqlash (Save)'}
                        </Button>
                      </div>
                    </div>
                  ) : (
