...
export default function HallDetailPage() {
  ...
  const loadCalendar = useCallback(async () => {
    ...
    try {
      ...
      const mapped = (data.bookings || []).map((b: any) => ({
        date: b.bookingDate,
        status: b.bookingStatus === 'cancelled' ? 'available' : 'booked',
      }))
    ...
    if (status === 'booked') {
      ...
      if (found && found.customerName) {
        setSelectedBookingDetails({
          ...
          customerPhone: found.customerPhone || 'N/A',
          guestCount: found.guestCount || 0,
        })
      }
    }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-rose-50/50 to-white dark:from-background dark:to-background pb-20 sm:pb-0">
                    ...
                    <Dialog open={reviewDialogOpen} onOpenChange={setReviewDialogOpen}>
                      ...
                      <DialogContent className="sm:max-w-md">
                        <DialogHeader>
                          ...
                          <DialogDescription>Share your experience with this wedding hall</DialogDescription>
                        </DialogHeader>
                ...
                <CardContent className="p-4">
                  ...
                  <div className="grid grid-cols-7 gap-1">
                    {calendarDays.map((day, index) => {
                      ...
                      const status = getDayStatus(day)
                      const isAuthorizedUser = user?.role === 'admin' || (user?.role === 'owner' && hall?.ownerId === user.userId)
                      const isClickable = status === 'available' || (status === 'booked' && isAuthorizedUser)
                      return (
                        <button
                          key={day}
                          ...
                          className={`
                            ...
                            ${status === 'booked' && `bg-red-100 text-red-800 border border-red-200 dark:bg-red-900/20 dark:text-red-300 dark:border-red-800 ${isAuthorizedUser ? 'cursor-pointer hover:scale-105 hover:bg-red-200 dark:hover:bg-red-900/30' : 'cursor-not-allowed'}`}
      ...
      {/* Booking Details Modal for Owner/Admin */}
      <Dialog open={!!selectedBookingDetails} onOpenChange={(open) => !open && setSelectedBookingDetails(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Tanlangan sana uchun bron ma&apos;lumotlari</DialogDescription>
          </DialogHeader>
          {selectedBookingDetails && (
            <div className="space-y-4 pt-4">
              <div className="flex justify-between items-center pb-2 border-b border-rose-100 dark:border-rose-900/30">
                <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">Sana:</span>
                <span className="text-sm font-bold dark:text-foreground">
              ...
              <div className="flex justify-between items-center pb-2 border-b border-rose-100 dark:border-rose-900/30">
                <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">Mijoz:</span>
                <span className="text-sm font-bold dark:text-foreground">{selectedBookingDetails.customerName}</span>
              </div>
              <div className="flex justify-between items-center pb-2 border-b border-rose-100 dark:border-rose-900/30">
                <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">Telefon:</span>
                <span className="text-sm font-bold text-rose-600 dark:text-rose-400">{selectedBookingDetails.customerPhone}</span>
              </div>
              <div className="flex justify-between items-center pb-2">
                <span className="text-sm font-semibold text-gray-500 dark:text-gray-400">Mehmonlar soni:</span>
                <span className="text-sm font-bold dark:text-foreground">{selectedBookingDetails.guestCount} ta o&apos;rindiq</span>
              </div>
              <Button
